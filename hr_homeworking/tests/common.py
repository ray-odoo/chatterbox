...
class TestHrHomeworkingCommon(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.unspecified_location_id = cls.env['homeworking.location']._get_default_homeworking_location_id()
        cls.office_location_id = cls.env['homeworking.location'].search([('location_type', '=', 'office')], limit=1).id
        cls.home_location_id = cls.env['homeworking.location'].search([('location_type', '=', 'home')], limit=1).id
